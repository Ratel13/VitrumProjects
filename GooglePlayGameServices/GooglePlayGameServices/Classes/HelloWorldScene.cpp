...
void HelloWorld::_onButtonSelected(CCObject *pSender)
{
    ...
    if(tag == 1)
    {
        // PlayGameSingleton::sharedInstance().showSingleLeaderboard(LEADERBOARD_HARD_MODE);
        PlayGameSingleton::sharedInstance().showLeaderboards();
    }
    else if(tag == 2)
    {
        PlayGameSingleton::sharedInstance().submitScore(3500 * CCRANDOM_0_1() + 1000, LEADERBOARD_EASY_MODE);
    }
    else if(tag == 3)
    {
    ...
    {
        PlayGameSingleton::sharedInstance().unlockAchievement(ACHIEVEMENT_003);
    }
}
